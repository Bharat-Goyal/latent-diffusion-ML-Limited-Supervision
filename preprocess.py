...
def transform(filename,indir,outdir):
  ...
  with torch.no_grad():
    ...
    np.save(filename.replace(indir,outdir) + '.spec.npy', spectrogram.cpu().numpy()) 
...
def spec_transform(filename,indir,outdir):
    ...
    np.save(filename.replace(indir,outdir) + '.spec.npy', spec)
...
def main(args):
  ...
  if args.se or args.voicebank:
    filenames = glob(args.dir + '/*.wav', recursive=True)
  else:
    filenames = glob(args.dir + '/*.Clean.wav', recursive=True)
